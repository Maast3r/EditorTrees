...
public class Node {
	...
	public Node delete(int pos) {
		...
		if (pos <= node.rank) {
			...
			// recurse right
			node.right.delete(pos - (node.rank + 1));
		}
	...
	public Node[] split(int pos) {
		// returns the two new roots of both trees
		...
		if (pos <= node.rank) {
			if (pos == node.rank) {
				...
				// these will be the new trees
				EditTree newLeftTree = new EditTree();
				EditTree newRightTree = new EditTree();

				newLeftTree = new EditTree(left);

				// add current to right result tree
				if (pos == 0) {
					newRightTree = new EditTree(node);
				} else {
					newRightTree = new EditTree(right);
					newRightTree.addToBeginning(node.element);
				}

				// newRightTree.delete(pos)
				// System.out.println("allah ahkbaar newleftTree: "
				// + newLeftTree.toString() + "     FOR ALLAH + "
				// + newRightTree.toString());
				...
				while (node.parent != NULL_NODE) {
					...
					EditTree leftTree = new EditTree(node.left);
					EditTree rightTree = new EditTree(node.right);

					Node temp;

					// determine which way to concatenate
					if (child == node.right) {
						if (leftTree.root.nodeHeight >= newLeftTree.root.nodeHeight) {
							temp = leftTree.pasteTreeL(newLeftTree, leftTree,
									node.element);
						} else {
							temp = newLeftTree.pasteTreeL(newLeftTree,
									leftTree, node.element);
						}
						newLeftTree = new EditTree(temp);
					} else {
						if (rightTree.root.nodeHeight >= newRightTree.root.nodeHeight) {
							temp = rightTree.pasteTreeR(newRightTree,
									rightTree, node.element);
						} else {
							temp = newRightTree.pasteTreeR(newRightTree,
									rightTree, node.element);
						}
						newRightTree = new EditTree(temp);
					}

				}
				Node[] twoRoots = { newLeftTree.root, newRightTree.root };
				return twoRoots;
			} else {
				// recurse left
				return node.left.split(pos);
			}
		} else {
			// recurse right
			return node.right.split(pos - (node.rank + 1));
		}
	...
	public int editTreeString(String left, String right) {
		// Assumes left and right heights are -1
		int leftH = -1;
		int rightH = -1;
		// Checks if left side is an empty string before running
		if (!left.isEmpty()) {
			// Divides string in half, creates a node from the middle, recurses
			// on both sides of newly created strings
			int midpointLeft = left.length() / 2;
			char midCharLeft = left.charAt(midpointLeft);
			this.left = new Node(midCharLeft, this.eTree);
			String newLeft = left.substring(0, midpointLeft);
			String newRight = left.substring(midpointLeft + 1);
			leftH = this.left.editTreeString(newLeft, newRight);

		}
		// same for right side
		if (!right.isEmpty()) {
			//Same code as above, just different variable names
			int midpointRight = right.length() / 2;
			char midCharRight = right.charAt(midpointRight);
			this.right = new Node(midCharRight, this.eTree);
			String newLeft = right.substring(0, midpointRight);
			String newRight = right.substring(midpointRight + 1);
			rightH = this.right.editTreeString(newLeft, newRight);
